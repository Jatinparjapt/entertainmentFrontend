...
// import {removeBookmarksDetailsSendToDatabase} from "../../Redux/bookMarks"
const BMovies = () => {
  AOS.init();
...
//  console.log(fetchBookmarkedMovies , " data bookmarkedMovies")
...
// show spiner until data not load full
if(!bookmarksMovies||bookmarksMovies === undefined){
  ...
  // console.clear();
  // find movie on user information
  const filteredMovies = bookmarksMovies.filter(movie =>
    movie.original_title.toLowerCase().includes(searchQuery.toLowerCase())
  );
  // const removeBookmarkMovies = (type ,id)=>{
  //   dispatch(removeBookmarksDetailsSendToDatabase({data:{media_type :type , media_id:id }}))
  //   // if(bookmarkResponse)
  // }
  return (
    <>
